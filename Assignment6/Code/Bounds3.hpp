                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    for(int i = 0; i < 3; i ++)
        if(!dirIsNeg[i]) std::swap(tmin[i], tmax[i]);   //Vector3f reloads the operator[]
    ...
    tenter = std::fmax(tmin.x, std::fmax(tmin.y, tmin.z));
    texit = std::fmin(tmax.x, std::fmin(tmax.y, tmax.z));
